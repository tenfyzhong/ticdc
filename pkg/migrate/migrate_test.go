...
func TestMigration(t *testing.T) {
	...
	status1 := config.ChangeFeedStatus{CheckpointTs: 1}
	info2 := config.ChangeFeedInfo{
		SinkURI: "test1",
		StartTs: 2, TargetTs: 200, State: config.StateWarning,
	}
	...
	info3 := config.ChangeFeedInfo{
		SinkURI: "test1",
		StartTs: 3, TargetTs: 300, State: config.StateFailed,
		Config: cfg,
	}
	status3 := config.ChangeFeedStatus{CheckpointTs: 3}
	...
	for _, tc := range testCases {
		infoResp, err := cli.Get(context.Background(),
			fmt.Sprintf("%s%s/%s", etcd.DefaultClusterAndKeyspacePrefix,
		...
		require.Equal(t, common.DefaultKeyspaceNamme, info.ChangefeedID.Keyspace())
		require.Equal(t, tc.id, info.ChangefeedID.Name())
		tc.info.ChangefeedID.DisplayName.Keyspace = info.ChangefeedID.Keyspace()
