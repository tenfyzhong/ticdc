...
type GlobalReactorState struct {
	...
	onCaptureAdded   func(captureID config.CaptureID, addr string)
	onCaptureRemoved func(captureID config.CaptureID)
...
func (s *GlobalReactorState) SetOnCaptureAdded(f func(captureID config.CaptureID, addr string)) {
	s.onCaptureAdded = f
}

// SetOnCaptureRemoved registers a function that is called when a capture goes offline.
func (s *GlobalReactorState) SetOnCaptureRemoved(f func(captureID config.CaptureID)) {
...
func NewChangefeedReactorState(clusterID string,
	...
	return &ChangefeedReactorState{
		ClusterID:     clusterID,
		ID:            id,
		TaskPositions: make(map[config.CaptureID]*config.TaskPosition),
	}
}

// GetID returns the changefeed ID.
func (s *ChangefeedReactorState) GetID() common.ChangeFeedID {
	return s.ID
}

// GetChangefeedInfo returns the changefeed info.
func (s *ChangefeedReactorState) GetChangefeedInfo() *config.ChangeFeedInfo {
	return s.Info
}

// GetChangefeedStatus returns the changefeed status.
func (s *ChangefeedReactorState) GetChangefeedStatus() *config.ChangeFeedStatus {
	return s.Status
}
...
func (s *ChangefeedReactorState) RemoveChangefeed() {
	// remove info
	s.PatchInfo(func(info *config.ChangeFeedInfo) (
		*config.ChangeFeedInfo, bool, error,
	) {
		return nil, true, nil
	})
	// remove changefeedStatus
	s.PatchStatus(
		func(status *config.ChangeFeedStatus) (
			*config.ChangeFeedStatus, bool, error,
		) {
			return nil, true, nil
		})
}
...
func (s *ChangefeedReactorState) ResumeChangefeed(overwriteCheckpointTs uint64) {
	...
	s.PatchStatus(func(status *config.ChangeFeedStatus) (
		*config.ChangeFeedStatus, bool, error,
	) {
		if overwriteCheckpointTs > 0 {
			...
			log.Info("overwriting the tableCheckpoint ts",
				zap.String("namespace", s.ID.Keyspace()),
...
func (s *ChangefeedReactorState) TakeProcessorErrors() []*config.RunningError {
	...
	for captureID, position := range s.TaskPositions {
		if position.Error != nil {
			...
			s.PatchTaskPosition(captureID, func(position *config.TaskPosition) (*config.TaskPosition, bool, error) {
				if position == nil {
					return nil, false, nil
				}
				position.Error = nil
...
func (s *ChangefeedReactorState) UpdateChangefeedState(feedState config.FeedState,
	adminJobType config.AdminJobType,
	epoch uint64,
) {
	s.PatchStatus(func(status *config.ChangeFeedStatus) (*config.ChangeFeedStatus, bool, error) {
		if status == nil {
			return status, false, nil
		}
...
func (s *ChangefeedReactorState) UpdateCDCKey(key *etcd.CDCKey, value []byte) error {
	...
	case etcd.CDCKeyTypeChangefeedInfo:
		...
		s.Info = new(config.ChangeFeedInfo)
		e = s.Info
	case etcd.CDCKeyTypeChangeFeedStatus:
		...
		s.Status = new(config.ChangeFeedStatus)
		e = s.Status
	case etcd.CDCKeyTypeTaskPosition:
		...
		position := new(config.TaskPosition)
...
func (s *ChangefeedReactorState) Active(captureID config.CaptureID) bool {
	return s.Info != nil && s.Status != nil && s.Status.AdminJobType == config.AdminNone
}
...
func (s *ChangefeedReactorState) CheckCaptureAlive(captureID config.CaptureID) {
	k := etcd.CDCKey{
...
func (s *ChangefeedReactorState) PatchInfo(fn func(*config.ChangeFeedInfo) (*config.ChangeFeedInfo, bool, error)) {
	key := &etcd.CDCKey{
		ClusterID:    s.ClusterID,
	...
	s.patchAny(key.String(), changefeedInfoTPI, func(e interface{}) (interface{}, bool, error) {
		...
		return fn(e.(*config.ChangeFeedInfo))
	})
}

// PatchStatus appends a DataPatch which can modify the ChangeFeedStatus
func (s *ChangefeedReactorState) PatchStatus(fn func(*config.ChangeFeedStatus) (*config.ChangeFeedStatus, bool, error)) {
	key := &etcd.CDCKey{
		ClusterID:    s.ClusterID,
	...
	s.patchAny(key.String(), changefeedStatusTPI, func(e interface{}) (interface{}, bool, error) {
		...
		return fn(e.(*config.ChangeFeedStatus))
	})
}

// PatchTaskPosition appends a DataPatch which can modify the TaskPosition of a specified capture
func (s *ChangefeedReactorState) PatchTaskPosition(captureID config.CaptureID, fn func(*config.TaskPosition) (*config.TaskPosition, bool, error)) {
	key := &etcd.CDCKey{
		ClusterID:    s.ClusterID,
	...
	s.patchAny(key.String(), taskPositionTPI, func(e interface{}) (interface{}, bool, error) {
		...
		return fn(e.(*config.TaskPosition))
	})
}

var (
	taskPositionTPI     *config.TaskPosition
	changefeedStatusTPI *config.ChangeFeedStatus
	changefeedInfoTPI   *config.ChangeFeedInfo
)
