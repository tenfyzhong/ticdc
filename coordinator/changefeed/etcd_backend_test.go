...
func TestPauseChangefeed(t *testing.T) {
	...
	changefeedID := common.NewChangeFeedIDWithName("test", common.DefaultKeyspaceID)
...
func TestUpdateChangefeedCheckpointTs(t *testing.T) {
	...
	cps := map[common.ChangeFeedID]uint64{
		common.NewChangeFeedIDWithName("test1", common.DefaultKeyspaceID): 100,
	}
	...
	for i := 0; i < 129; i++ {
		cps[common.NewChangeFeedIDWithName(fmt.Sprintf("%d", i), common.DefaultKeyspaceID)] = 100
	}
