...
# This is the case-D of fail-over with ddl events.
...
function failOverCaseD-1() {
	...
	echo "failOverCaseD-1 passed successfully"
}

# ddl is drop table
function failOverCaseD-2() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseD-2 passed successfully"
}

# ddl is rename table
function failOverCaseD-3() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseD-3 passed successfully"
}
