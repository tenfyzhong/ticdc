...
# This is the case-N of fail-over with ddl events.
...
# the node with table trigger(Node1) is restarted, the other node(Node2) is not restarted.
...
function failOverCaseN-1() {
	...
	export GO_FAILPOINTS='github.com/pingcap/ticdc/maintainer/scheduler/StopBalanceScheduler=return(true);github.com/pingcap/ticdc/downstreamadapter/dispatcher/BlockOrWaitBeforeDealWithDDL=1*sleep(30000)->return(false)'
	...
	echo "failOverCaseN-1 passed successfully"
}

# ddl is drop table
function failOverCaseN-2() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseN-2 passed successfully"
}

# ddl is rename table
function failOverCaseN-3() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseN-3 passed successfully"
}

# ddl is truncate table
function failOverCaseN-4() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseN-4 passed successfully"
}
