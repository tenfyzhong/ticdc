...
# This is the case-O of fail-over with ddl events.
# Node1: the table trigger event dispatcher meet the block event ddl, and report the status to maintainer,
# Node2: the other dispatcher is not meet the block event ddl yet,
# Then the node with the other dispatcher(Node2) is restarted, the other node with table trigger(Node1) is not restarted.
...
function failOverCaseO-1() {
	...
	export GO_FAILPOINTS='github.com/pingcap/ticdc/maintainer/scheduler/StopBalanceScheduler=return(true);github.com/pingcap/ticdc/downstreamadapter/dispatcher/BlockOrWaitBeforeDealWithDDL=sleep(30000)'
	...
	echo "failOverCaseO-1 passed successfully"
}

# ddl is drop table
function failOverCaseO-2() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseO-2 passed successfully"
}

# ddl is rename table
function failOverCaseO-3() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseO-3 passed successfully"
}

# ddl is truncate table
function failOverCaseO-4() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseO-4 passed successfully"
}
