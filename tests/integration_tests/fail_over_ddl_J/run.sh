...
# This is the case-J of fail-over with ddl events.
...
# Then the node with the other dispatcher(Node2) is restarted, the node with maintainer and table trigger event dispatcher(Node1) is not restarted.
...
function failOverCaseJ-1() {
	...
	export GO_FAILPOINTS='github.com/pingcap/ticdc/maintainer/scheduler/StopBalanceScheduler=return(true);github.com/pingcap/ticdc/downstreamadapter/dispatcher/BlockOrWaitBeforePass=pause'
