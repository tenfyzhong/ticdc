...
function run() {
	...
	TOPIC_NAME="canal-json-handle-key-only-$RANDOM"
	...
	SINK_URI="kafka://127.0.0.1:9092/$TOPIC_NAME?protocol=canal-json&enable-tidb-extension=true&max-message-bytes=1000&kafka-version=${KAFKA_VERSION}"
