...
function failOverCaseG-1() {
	...
	export GO_FAILPOINTS='github.com/pingcap/ticdc/downstreamadapter/dispatcher/BlockOrWaitBeforePass=pause'
	...
	echo "failOverCaseG-1 passed successfully"
}

# ddl is drop table
function failOverCaseG-2() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseG-2 passed successfully"
}

# ddl is rename table
function failOverCaseG-3() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
	...
	echo "failOverCaseG-3 passed successfully"
}

# ddl is truncate table
function failOverCaseG-4() {
	prepare
	ret=$?
	if [ "$ret" != 0 ]; then
		exit 1
