...
function run() {
	...
	# up -> down
	create_changefeed --sink-uri="$SINK_URI_1" -c "test-1" --config="$CUR/conf/up.toml"
	# down -> up
	create_changefeed --sink-uri="$SINK_URI_2" -c "test-2" --server "http://127.0.0.1:8400" --config="$CUR/conf/down.toml" --next-gen 0
