...
func (oc *Controller) pollQueueingOperator() (
	...
	if op.IsFinished() {
		...
		metrics.FinishedOperatorCount.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type()).Inc()
		metrics.OperatorDuration.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type()).Observe(time.Since(item.CreatedAt).Seconds())
...
func (oc *Controller) pushOperator(op operator.Operator[common.DispatcherID, *heartbeatpb.TableSpanStatus]) {
	...
	metrics.CreatedOperatorCount.WithLabelValues(common.DefaultKeyspace, oc.changefeedID.Name(), op.Type()).Inc()
}
